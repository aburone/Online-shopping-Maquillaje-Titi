...
class Operation
  attr_reader :loc, :orig, :dest, :ammount, :order
  def initialize(loc, orig, dest, ammount, order)
    @loc = loc
    @orig = orig
    @dest = dest
    @ammount = ammount
...
class Transaction
  ...
  def to_s
    ...
    @operations.map do |o|
      out += "\n#{o.orig} #{l o.ammount} -> #{o.dest} (#{l o.ammount}) "
  ...
  def add(loc: nil, orig:, dest:, ammount:, order: nil)
    ...
    operation = Operation.new(loc, orig, dest, ammount, order)
    @operations << operation
    @total += ammount if orig == "Caja" or orig == "Banco"
    @total -= ammount if dest == "Caja" or dest == "Banco"
  end
