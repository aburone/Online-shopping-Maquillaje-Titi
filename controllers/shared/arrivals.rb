  ...
  def verify order, type, id = nil
    ...
    case order.type
      ...
      when Order::POS_TO_WH 
        slim :verify_transport_order, layout: :layout_backend, locals: {sec_nav: :nav_logistics} 
      when Order::WH_TO_WH 
        slim :verify_transport_order, layout: :layout_backend, locals: {sec_nav: :nav_logistics} 
    end
  end
...
class Sales < AppController
  ...
  post '/transport/arrivals/:o_id/:i_id/void' do 
    ...
    slim :void_item, layout: :layout_sales, locals: {base_route: "/transport/arrivals"}
  end
...
class Backend < AppController
  ...
  route :get, :post, ['/transport/departures/wh_to_wh/:o_id/item/remove', '/transport/departures/wh_to_pos/:o_id/item/remove'] do
    @order = get_orders_at_location_with_type_status_and_id_or_redirect current_location[:name], o_type_from_route, Order::OPEN, params[:o_id].to_i, "/transport/departures/#{o_type_from_route}/select"
    if params[:id].nil?
      slim :remove_item, layout: :layout_backend, locals: {sec_nav: :nav_logistics, action_url: "/transport/departures/#{@order.type.downcase}/#{@order.o_id}/item/remove", title: t.production.remotion.title(@order.o_id)}
    else
      item = get_item_for_removal params[:id], @order
      redirect_if_has_errors item, "/transport/departures/#{@order.type.downcase}/#{@order.o_id}/add" 
      remove_item_from_order @order, item
  ...
  route :get, :post, ['/transport/departures/wh_to_wh/:o_id/bulk/remove'] do
    ...
    if params[:id].nil?
      slim :remove_item, layout: :layout_backend, locals: {sec_nav: :nav_logistics, action_url: "/transport/departures/wh_to_wh/#{@order.o_id}/bulk/remove", title: t.production.bulk_remotion.title(@order.o_id)}
    else
  ...
  get '/transport/pending' do
    ...
    slim :pending_arrivals_verifications, layout: :layout_backend, locals: {sec_nav: :nav_logistics, include_bulks: true}
  end
  ...
  route :get, ["/transport/arrivals", "/transport/arrivals/select"] do
    ...
    slim :orders_list, layout: :layout_backend, locals: {title: "Ingresos", sec_nav: :nav_logistics, full_row: true, list_mode: "transport", can_edit: true, edit_link: "/admin/transport/arrivals/o_id"}
  end
  ...
  post '/transport/arrivals/:o_id/:i_id/void' do 
    ...
    slim :void_item, layout: :layout_backend, locals: {base_route: "/admin/transport/arrivals"}
  end
