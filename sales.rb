...
class Sales < AppController
  ...
  get '/products' do
    ...
    slim :products, layout: :layout_sales, locals: {full_row: true, stock_col: true, show_edit_button: false, show_filters: true, sec_nav: :nav_products, title: t.products.title}
  end

  get '/products/items/?' do
    @items = Item.new.get_items_at_location_with_status current_location[:name], Item::READY
    slim :items, layout: :layout_sales, locals: {show_edit_button: false, show_filters: true, sec_nav: :nav_products, title: "Items disponibles"}
  end
